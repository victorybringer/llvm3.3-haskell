...
--  Just name <- cValueName vp
  ...
  uid <- nextId
  name <- cValueNameWith makeGlobalIdentifier uid vp
...
--  Just name <- cValueName vp
  ...
  mds <- mapM (translateMetadata finalState) metaPtr
  uid <- nextId
  name <- cValueNameWith makeLocalIdentifier uid vp
...
------ zyz
cValueNameWith mkIdent uid vp = do
  name' <- cValueName vp
  let name =  fromMaybe nullName name'
      nullName = mkIdent (pack $ "Unknown_" ++ show uid)
  return name 