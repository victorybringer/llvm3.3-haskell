...
visualizeGraph inFile outFile fmt optOptions fromModule  = do
  ...
  visualizeGraphFromModule m outFile fmt fromModule


visualizeGraphFromModule :: (ToGraphviz a)
                  => Module -- ^ Input Module
                  -> Maybe FilePath -- ^ Output file name
                  -> OutputType -- ^ Type of output requested
                  -> (Module -> [(String, a)]) -- ^ A function to turn a Module into some graphs
                  -> IO ()
visualizeGraphFromModule m outFile fmt fromModule  = do
--  let p = parseLLVMFile defaultParserOptions
--  m <- buildModule [] optOptions p inFile
  let gs = fromModule m
      outFile' = case outFile of
          Nothing     -> takeBaseName . T.unpack $ moduleIdentifier m
          Just fn     -> fn
