...
import LLVM.Slicing.Util.Utils  ( getFunctionLine,toValName,valueLine )
...
gUnion :: SDGGraphType -> SDGGraphType -> SDGGraphType
gUnion g1 g2 = mkGraph (nub $ labNodes g1 ++ labNodes g2)(nub $ labEdges g1 ++ labEdges g2)
{-# INLINE gUnion #-}

gEqual g1 g2 = noNodes g1 == noNodes g2 && edges g1 == edges g2

showNode :: (Int -> Value) -> SDGNode -> String
showNode i2v (InstructionNode i) = show (i2v i)
showNode i2v (ActualInNode i v) = printf "(actual-in)%s:%s" (toValName $ i2v i)(toValName $ i2v v)
showNode i2v (ActualOutNode i v) = printf "(actual-out)%s:%s"(toValName $ i2v i)(toValName $ i2v v)
showNode i2v (FormalInNode a) = printf "(formal-in)%s". toValName $ i2v a
showNode i2v (FormalOutNode a) = printf "(formal-out)%s". toValName $ i2v a
showNode i2v (FunctionEntryNode f) = printf "ENTER %s". toValName $ i2v f
showNode i2v (ExternalFunctionNode f) = printf "ExternalFunc %s". toValName $ i2v f
showNode i2v (GlobalVariableInNode f g) = printf "(global-in)%s:%s" (toValName $ i2v f)(toValName $ i2v g)
showNode i2v (GlobalVariableOutNode f g) = printf "(global-out)%s:%s" (toValName $ i2v f)(toValName $ i2v g)
showNode i2v (FinalUseNode u) = printf "FinalUse(%s)". toValName $ i2v u
...
instance Show SDGCluster where
  ...
  show (CFunction f) = showFunSrcLn f ++ show (functionName f)
...
sdgParams =
  ...
  where
    ...
    formatCluster (CFunction f) = [GraphAttrs { attrs = [toLabel (showFunSrcLn f ++ show (functionName f))]} ]
...
-----------------------------------------------------
---
showFunSrcLn f = show [fn ++ ":" ++ show ln | (fn,ln) <- maybeToList (getFunctionLine f)] 
...
insEdges2' es g = F.foldr' insEdge2' g es
  where  
    insEdge2' :: (Show b, DynGraph gr) => LEdge b -> gr a b -> gr a b
    insEdge2' n@(v,w,l) g = 
      case (gelem v g,gelem w g) of
        (True,True) -> insEdge n g
        _ ->  g   
--    (False,True,True,_) -> insEdge (-v,w,l) g
--    (True,_,False,True) ->  insEdge (v,-w,l) g
--    (False,True,False,True) -> insEdge (-v,-w,l) g
{-# INLINE insEdges2' #-}
